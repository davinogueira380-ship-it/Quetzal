...
{
    public partial class FormProjetoC : Form
    {
        ...
        public class FotoItemModel
        {
            ...
            public override string ToString() => $"[{AmbienteNome}] {Path.GetFileName(CaminhoArquivo)}";
        }
        ...
        private async void FormProjetos_Load(object sender, EventArgs e)
        {
        ...
        private async Task CarregarDadosIniciaisAsync()
        {
            try
            {
                ...
                try
                {
                    ...
                    cmbCliente.Items.Add(new ItemCombo { Id = "", Texto = "-- Selecione o Cliente --" });
                    foreach (var c in _listaClientes)
                    {
                        var status = c.Ativo ? "" : " (Inativo)";
                        cmbCliente.Items.Add(new ItemCombo { Id = c.Id, Texto = $"{c.NomeCompleto}{status}" });
                    }
                ...
                try
                {
                    ...
                    {
                        clbAmbientes.Items.Add(new ItemCombo { Id = amb.Id.ToString(), Texto = amb.Nome });
                        cmbAmbienteFoto.Items.Add(new ItemCombo { Id = amb.Id.ToString(), Texto = amb.Nome });
                    }
                    if (cmbAmbienteFoto.Items.Count > 0) cmbAmbienteFoto.SelectedIndex = 0;
                }
                catch { }
            ...
            {
                MessageBox.Show($"Erro ao carregar dados do formulário: {ex.Message}", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        private async Task CarregarProjetosAsync()
        {
            try
            {
                ...
                _listaProjetos = await _apiProjeto.ObterTodosAsync(incluirInativos: true);
                AtualizarGrid(_listaProjetos);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Não foi possível carregar a lista de projetos: {ex.Message}", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            finally
            {
        ...
        {
            if (dgvProjetos.SelectedRows.Count == 0) return;
            ...
            if (linha.DataBoundItem is ProjetoCDto item)
            {
                ...
                {
                    if (cmbCliente.Items[i] is ItemCombo combo && combo.Id == item.UsuarioId)
                    {
                ...
                for (int i = 0; i < clbAmbientes.Items.Count; i++)
                {
                    ...
                    {
                        var marcado = item.AmbientesIds != null && item.AmbientesIds.Contains(int.Parse(combo.Id))
                ...
                if (item.Fotos != null)
                {
                    ...
                    {
                        _fotosDoProjeto.Add(new FotoItemModel
                        {
                ...
                btnDesativar.Text = item.Ativo ? "🗑️ Desativar Projeto" : "🔄 Reativar Projeto";
            }
        }
        ...
        {
            if (cmbAmbienteFoto.SelectedItem is not ItemCombo ambienteSelecionado || string.IsNullOrEmpty(ambienteSelecionado.Id))
            {
                MessageBox.Show("Selecione qual ambiente esta foto representa antes de anexar.", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    AtualizarListaFotos();
                    lstFotos.SelectedIndex = _fotosDoProjeto.Count - 1;
                }
                catch (Exception ex)
                {
        ...
        private void lstFotos_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (lstFotos.SelectedItem is FotoItemModel foto)
            {
                ...
                else if (!string.IsNullOrEmpty(foto.CaminhoArquivo) && File.Exists(foto.CaminhoArquivo))
                {
                    try { picPreviewFoto.Image = Image.FromFile(foto.CaminhoArquivo); } catch { }
                }
                else
                {
                    picPreviewFoto.Image = null;
        ...
        {
            if (lstFotos.SelectedIndex >= 0 && lstFotos.SelectedIndex < _fotosDoProjeto.Count)
            {
        ...
        private async void btnSalvar_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Informe o nome do projeto.", "Campo Obrigatório", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtNomeProjeto.Focus();
                return;
            }

            var clienteCombo = cmbCliente.SelectedItem as ItemCombo;
            var clienteId = clienteCombo?.Id ?? "";
            if (string.IsNullOrWhiteSpace(clienteId))
            {
                MessageBox.Show("Selecione a qual cliente este projeto pertence.", "Campo Obrigatório", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                cmbCliente.Focus();
                return;
            }
            ...
            for (int i = 0; i < clbAmbientes.Items.Count; i++)
            {
                if (clbAmbientes.GetItemChecked(i) && clbAmbientes.Items[i] is ItemCombo combo)
                {
            ...
            try
            {
                ...
                var fotosDto = _fotosDoProjeto.Select(f => new FotoAmbienteDto
                {
                    AmbienteId = f.AmbienteId,
                    AmbienteNome = f.AmbienteNome,
                    CaminhoOuBase64 = f.Base64,
                    Descricao = $"Foto do ambiente {f.AmbienteNome}"
                }).ToList();
                ...
                if (_projetoSelecionadoId == null || _projetoSelecionadoId == 0)
                {
                    var resposta = await _apiProjeto.CadastrarAsync(dto);
                    MessageBox.Show("Projeto do cliente cadastrado com sucesso com seus ambientes e galeria de fotos!", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    var resposta = await _apiProjeto.AtualizarAsync(_projetoSelecionadoId.Value, dto);
                    MessageBox.Show("Projeto do cliente atualizado com sucesso!", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            ...
            {
                MessageBox.Show($"Erro ao salvar projeto: {ex.Message}", "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            finally
            {
        ...
        {
            if (_projetoSelecionadoId == null) return;
            ...
            if (confirmacao == DialogResult.Yes)
            {
                try
                {
                    await _apiProjeto.DesativarAsync(_projetoSelecionadoId.Value);
                    MessageBox.Show("Situação do projeto alterada com sucesso!", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    await CarregarProjetosAsync();
                    LimparCampos();
                }
                catch (Exception ex)
        ...
        private void txtBusca_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                var filtrados = _listaProjetos
                    .Where(p => (p.Nome?.ToLower().Contains(termo) ?? false) ||
