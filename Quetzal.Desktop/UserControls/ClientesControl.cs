...
namespace Quetzal.Desktop.UserControls
{
    public partial class ClientesControl : UserControl
    {
        ...
        private async void ClientesControl_Load(object sender, EventArgs e)
        {
        ...
        private async void btnSalvar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                await _apiUsuario.AtualizarAsync(
                    _clienteSelecionadoId,
                    cliente);

                // Verifica se o status de ativação mudou
                if (clienteAtual != null &&
                    clienteAtual.Ativo != swPerfilAtivo.Checked)
                {
                    if (swPerfilAtivo.Checked)
                    {
                        await _apiUsuario.AtivarAsync(
                            _clienteSelecionadoId);
                    }
                    else
                    {
                        await _apiUsuario.DesativarAsync(
                            _clienteSelecionadoId);
                    }
                }
        ...
        private async void btnAlternarAtivacao_Click(object sender, EventArgs e)
        {
            ...
            var acaoTexto = novoStatus
                ? "ATIVAR o perfil e LIBERAR o acesso às fotos"
                : "DESATIVAR o perfil e BLOQUEAR o acesso às fotos";
            ...
            try
            {
                if (novoStatus)
                {
                    ...
                    MessageBox.Show(
                        "Perfil ativado! O cliente agora consegue visualizar as fotos na área do cliente.",
                        "Perfil Ativado",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Information);
                }
                else
                {
                    ...
                    MessageBox.Show(
                        "Perfil desativado! O cliente foi bloqueado de visualizar fotos até ser reativado.",
                        "Perfil Desativado",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Information);
                }
            ...
            {
                MessageBox.Show(
                    $"Erro ao alternar status do cliente: {ex.Message}",
                    "Erro",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
            }
